    ...
    public static function handle_export_logs() {
        ...
        if ( ! empty( $rows ) ) {
            ...
            $headers = array_map( function( $h ) {
                return $h === 'created_at' ? 'created_at_utc' : $h;
            }, $headers );

            $headers[] = 'created_at_local';

            fputcsv( $output, $headers, ',', '"', '\\' );

            foreach ( $rows as $row ) {
                $row['created_at_utc'] = $row['created_at'];
                unset( $row['created_at'] );

                $timestamp = strtotime( $row['created_at_utc'] . ' UTC' );
                $row['created_at_local'] = wp_date(
                    get_option( 'date_format' ) . ' ' . get_option( 'time_format' ),
                    $timestamp
                );
